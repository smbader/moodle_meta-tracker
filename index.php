                    ...
                    <div class="card-body">
                        ...
                        if ($mysqli->connect_errno) {
                            ...
                            if (!$JIRA_DOMAIN || !$JIRA_EMAIL || !$JIRA_API_TOKEN) {
                                ...
                                $stmt = $mysqli->prepare("SELECT keyname, source_type, title, status, reporter FROM saved_issues WHERE user_id = ?");
                                $stmt->bind_param("i", $user_id);
                                $stmt->execute();
                                $result = $stmt->get_result();
                                $rowCount = $result ? $result->num_rows : 0;
                                // Remove debug output
                                echo "<table class='table table-striped table-bordered'><thead><tr><th>Issue Key</th><th>Status</th><th>Reporter</th><th>Latest Comment</th></tr></thead><tbody>";
                                if ($result && $result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        $reporter = htmlspecialchars($row['reporter'] ?? '');
                                        if ($sourceType === 'jira') {
                                            ...
                                            curl_close($ch);
                                            echo "<tr><td style='white-space:nowrap;'><a href='$jiraUrl' target='_blank'><strong>$issueKey</strong></a></td><td>$status</td><td>$reporter</td><td>$latestComment</td></tr>";
                                        } else if ($sourceType === 'github') {
                                            // Parse owner/repo#number (fix regex: escape # as \#)
                                            if (preg_match('#^([\w\-\.]+)/([\w\-\.]+)\#(\d+)$#', $row['keyname'], $m)) {
                                                $owner = $m[1];
                                                $repo = $m[2];
                                                $number = $m[3];
                                                $githubUrl = "https://github.com/$owner/$repo/issues/$number";
                                                echo "<tr><td style='white-space:nowrap;'><a href='$githubUrl' target='_blank'><strong>$issueKey</strong></a></td><td>$status</td><td>$reporter</td><td>N/A</td></tr>";
                                            } else {
                                                echo "<tr><td>$issueKey</td><td>$status</td><td>$reporter</td><td><span class='text-danger'>Invalid GitHub key format</span></td></tr>";
                                            }
                                        }
                                    }
                                } else {
                                    echo "<tr><td colspan='4' class='text-center text-muted'>No saved tracker items found.</td></tr>";
                                }
                                echo "</tbody></table>";
                                $stmt->close();
                            }
